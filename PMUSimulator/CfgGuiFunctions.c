...
int validation_new_data_rate (GtkButton *but, gpointer udata)
{
	...
	text  = gtk_combo_box_get_active_text(GTK_COMBO_BOX(p_drate));
...
void channel_names_for_phasor ()
{
	...
	for(i=1, j=1; i<=temp_local; j++)
	{
		...
		//gtk_table_attach_defaults (GTK_TABLE (table), num_text1[j], 1, 2, i+1, i+2);
		...
		label = gtk_label_new (line);
		//gtk_table_attach_defaults (GTK_TABLE (table), label, 2, 3, i+1, i+2);
...
//	Removebutton = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (Addbutton), "Remove Channels");
...
//	Removebutton = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (Addbutton), "NO");
	gtk_table_attach_defaults (GTK_TABLE (table), Removebutton, 0, 1, 3, 4);
//   gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (Removebutton), TRUE);
	gtk_widget_show (Removebutton);
