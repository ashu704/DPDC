...
void recreate_cfg_objects(){ 
	...
	/* Create CFG Objects from dpdc?.bin*/
	FILE *file = fopen (dpdcFilePath,"rb");

	if(file != NULL)
	{
		tempi = 1;

		while(tempi < 8)    //DPDCServer
		{
	...
	{
		printf("DPDC Setup File is missing, DPDC exit.\n");
		exit(1);
	}
} 
...
void init_cfgparser(unsigned char st[]){ 
	...
	while(j<num_pmu) {
		...
		printf("CFG consist Phasor = %d, Analogs = %d, and Digitals = %d.\n",phn,ann,dgn);
...
/* It re-creates objects by reading the file dpdcINFO.bin.*/
/* ----------------------------------------------------------------------------	*/
