...
/* This function will perform the writing of DPDC Setup File. It will simply	*/
...
void write_cfg_to_file()
{
	...
	if(cfgfirst != NULL)
	{
		fp = fopen(dpdcFilePath,"rb");

		strcpy(fileName, dpdcFolderPath);
		strcat(fileName, "/tempFile.bin");

		fp1 = fopen(fileName, "wb");

		for(tmpi=6; tmpi>0; tmpi--)
		{
		...
		if (remove(dpdcFilePath) == -1)
			perror("Error in deleting a file");
		else
			rename(fileName, dpdcFilePath);
	}
